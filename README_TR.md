# SOYO Tam Otomatik v2.5

Bu sürümde manuel onay ve otomasyon aç/kapa kutuları kaldırıldı. Akış tek modda, sürekli tam otomatik çalışır.
...
5. Üstten alta ilk uygun ve daha önce açılmamış satırın sohbetini açar.
6. Sohbet adını tekrar doğrular, mesajı yazar ve gönder düğmesine **yalnızca bir kez** basar.
7. Gönderimden sonra Önerilenler'e döner ve sıradaki uygun satırla devam eder. Görünür uygun kişi kalmadığında listeyi kaydırır.
8. Bir öneri satırı bir kez açıldıktan sonra aynı servis oturumunda yeniden seçilmez. Liste adı ile sohbet başlığı farklı biçimde yazılsa bile tıklanan liste satırı ayrıca tamamlanmış kabul edilir.
...
10. Manuel `ONAYLA` ekranı yoktur; tam otomatik çalışma kapatılamaz.

## v2.5 sohbet takılma / kaydırma düzeltmesi

- Yeni sohbet açıldıktan sonra mesaj kutusu veya gönder düğmesi Flutter erişilebilirlik ağacına geç düşerse servis sohbet gövdesinde küçük bir otomatik kaydırma yapıp arayüzü yeniden tarar; elle kaydırma ihtiyacı azaltıldı.
- Gönder düğmesi ilk denemelerde görünmüyorsa kör koordinat tıklaması hemen yapılmaz; önce otomatik UI uyandırma hareketleri denenir.
- Aynı öneri satırı artık isim dışında satırın yakınındaki görünen bilgilerden üretilen parmak iziyle de takip edilir; aynı satıra geri girme riski azaltıldı.
- Aynı ada sahip iki farklı öneri satırı, parmak izleri farklıysa birbirini yanlışlıkla engellemez.
- Görünür uygun satır kalmadığında ilk otomatik liste kaydırması güçlendirildi; küçük gesture'ın Flutter tarafından yutulması azaltıldı.
- Sürüm `2.5` / `versionCode 16`.

## v2.4 aynı sohbete dönme / çift gönderme düzeltmesi

- Aynı öneri satırının yeniden açılması engellendi.
- Liste satırındaki isim ile sohbet başlığındaki isim farklı normalize edilse bile ikisi birlikte işlendi olarak tutulur.
- Gönder tıklaması Android tarafından bir kez kabul edildikten sonra doğrulama gecikse bile aynı mesaj için ikinci kez gönder düğmesine basılmaz.
- Manuel onay akışı ve otomasyon aç/kapa kutuları kaldırıldı; eski kapalı tercihler açılışta otomatik açık yapılır.
- Sürüm `2.4` / `versionCode 15`.
