...
public class SoyoAccessibilityService extends AccessibilityService {
    ...
    // Önerilenler listesinden gerçekten tıklanan satırın adı. Sohbet başlığı farklı
    // normalize edilse bile liste satırını ayrıca tamamlandı saymak için korunur.
    private String activeRecommendationName;
    private String activeRecommendationFingerprint;
    // Aynı servis oturumunda bir kere açılmış satırı tekrar açma. Bu, liste adı ile
    // sohbet başlığı farklı görünse bile aynı profile dönüp durmayı engeller.
    private final Set<String> openedRecommendationNames = new HashSet<>();
    // Sadece isme güvenme: aynı öneri satırının yakınındaki görünen metinlerden de kararlı
    // bir parmak izi tut. SOYO adı farklı biçimde çizse bile aynı satıra tekrar girilmez.
    private final Set<String> openedRecommendationFingerprints = new HashSet<>();
    private final Set<String> handledRecommendationFingerprints = new HashSet<>();
    // Gönder düğmesine bir kez tıklama kabul edildiyse o sohbet için ikinci kez basma.
    private boolean sendClickAcceptedForPending = false;
    // SOYO/Flutter bazen yeni sohbet ekranını açıyor ama mesaj kutusu / gönder düğümü
    // erişilebilirlik ağacına ancak kullanıcı ekranda küçük bir kaydırma yapınca düşüyor.
    // Bunu otomatik olarak tetiklemek için sohbet açılışını ve son "uyandırma" hareketini izleriz.
    private long chatOpenedAt = 0L;
    private long lastChatNudgeAt = 0L;
    private int chatNudgeCount = 0;
    ...
    private void scanSoyoScreen() {
        ...
        try {
            ...
            // Yeni sohbet açılmış olsa bile Flutter bazen composer düğümlerini erişilebilirlik
            // ağacına ancak ekranda bir kaydırma olunca ekliyor. Kullanıcının elle kaydırmasına
            // ihtiyaç kalmaması için sohbet açılışında en fazla iki küçük otomatik "uyandırma"
            // hareketi yap; Önerilenler ekranındaysak bu yol hiç çalışmaz.
            if (maybeNudgeOpenedChat(root)) return;
    ...
    private void maybeOpenRecommendation(AccessibilityNodeInfo root) {
        ...
        try {
            ...
            activeRecommendationName = target.name;
            activeRecommendationFingerprint = target.fingerprint;
            ...
            if (clicked) {
                openedRecommendationNames.add(nameKey(target.name));
                if (!TextUtils.isEmpty(target.fingerprint)) {
                    openedRecommendationFingerprints.add(target.fingerprint);
                }
                chatOpenedAt = now;
                lastChatNudgeAt = 0L;
                chatNudgeCount = 0;
                ...
                activeRecommendationName = null;
                activeRecommendationFingerprint = null;
    ...
    private boolean maybeNudgeOpenedChat(AccessibilityNodeInfo root) {
        if (!isFullAutoEnabled() || root == null || !openingRecommendation
                || TextUtils.isEmpty(carriedRecommendationName)) return false;

        long now = SystemClock.elapsedRealtime();
        if (chatOpenedAt <= 0L) chatOpenedAt = lastRecommendationClickAt;
        if (now - chatOpenedAt < 520L) return false;
        if (chatNudgeCount >= 2 || now - lastChatNudgeAt < 650L) return false;

        List<AccessibilityNodeInfo> nodes = new ArrayList<>();
        collectNodes(root, nodes, 0);
        boolean recommendations = looksLikeRecommendationsScreen(nodes);
        recycleNodes(nodes);
    ...
    private boolean nudgeChatUi() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) return false;
        int width = getResources().getDisplayMetrics().widthPixels;
        int height = getResources().getDisplayMetrics().heightPixels;

        // Mesaj kutusuna ya da üst başlığa dokunmadan, sohbet gövdesinde çok küçük bir
        // yukarı kaydırma. Amaç konuşmayı taşımak değil Flutter'ın node ağacını tazelemek.
        float x = width * 0.50f;
        float startY = height * 0.63f;
        float endY = height * 0.51f;
        Path path = new Path();
    ...
    private boolean swipeRecommendationsUp(int recoveryLevel, int stagnantScans) {
        ...
        if (recoveryLevel >= 2) {
            ...
            // Ekranda açılabilir satır kalmadıysa eski kısa hareket bazı Flutter
            // sürümlerinde hiç scroll üretmiyordu. İlk denemeyi de belirgin yap.
            startY = height * 0.80f;
            endY = height * 0.52f;
            duration = 230;
        }
    ...
    private RecommendationTarget findFirstRecommendationTarget(List<AccessibilityNodeInfo> nodes) {
        ...
        for (AccessibilityNodeInfo chatNode : nodes) {
            ...
            String fingerprint = buildRecommendationRowFingerprint(nodes, chatRect, rowName.name);
            boolean hasFingerprint = !TextUtils.isEmpty(fingerprint);
            boolean seenFingerprint = hasFingerprint
                    && (openedRecommendationFingerprints.contains(fingerprint)
                    || handledRecommendationFingerprints.contains(fingerprint));
            // Parmak izi varsa asıl kimlik odur. Böylece aynı ada sahip iki farklı kişi
            // birbirini yanlışlıkla engellemez. Parmak izi üretilemezse eski isim koruması
            // güvenli geri dönüş olarak kalır.
            boolean seenByFallbackName = !hasFingerprint
                    && (handledRecommendationNames.contains(nameKey(rowName.name))
                    || openedRecommendationNames.contains(nameKey(rowName.name)));
            if (seenFingerprint || seenByFallbackName) continue;
    ...
    private String buildRecommendationRowFingerprint(List<AccessibilityNodeInfo> nodes, Rect chatRect, String rowName) {
        if (nodes == null || chatRect == null || chatRect.isEmpty()) return nameKey(rowName);
        int rowTolerance = Math.max(dp(34), Math.min(dp(52), chatRect.height() + dp(16)));
        List<String> tokens = new ArrayList<>();
        for (AccessibilityNodeInfo node : nodes) {
            if (node == null || !node.isVisibleToUser()) continue;
            Rect r = new Rect();
            node.getBoundsInScreen(r);
            if (r.isEmpty() || Math.abs(r.centerY() - chatRect.centerY()) > rowTolerance) continue;
            String raw = nodeText(node).trim();
            if (raw.isEmpty()) raw = safe(node.getContentDescription()).trim();
            if (raw.isEmpty()) continue;
            String token = Normalizer.normalize(raw, Normalizer.Form.NFKC);
            token = foldStylizedLatin(token).toLowerCase(new Locale("tr", "TR"));
            token = token.replaceAll("\\s+", " ").trim();
            if (token.isEmpty() || isChatLabel(token)) continue;
            if (token.length() > 36) token = token.substring(0, 36);
            if (!tokens.contains(token)) tokens.add(token);
        }
        java.util.Collections.sort(tokens);
        StringBuilder out = new StringBuilder(nameKey(rowName));
    ...
    private void prepareChatMessage(AccessibilityNodeInfo root, AccessibilityNodeInfo editor) {
        ...
        // Ancak aktif öneri satırının farklı bir parmak izi varsa yalnızca aynı ada sahip başka
        // bir kullanıcı olabilir; isim çakışması yüzünden onu yanlışlıkla atlama.
        boolean activeRowAlreadyHandled = !TextUtils.isEmpty(activeRecommendationFingerprint)
                && handledRecommendationFingerprints.contains(activeRecommendationFingerprint);
        boolean noRowIdentity = TextUtils.isEmpty(activeRecommendationFingerprint);
        if (isFullAutoEnabled() && handledRecommendationNames.contains(nameKey(candidate.name))
                && (activeRowAlreadyHandled || noRowIdentity)) {
            recycleNode(editor);
        ...
        sendClickAcceptedForPending = false;
    ...
    private void skipCurrentChatAndContinue(String handledName) {
        ...
        if (!TextUtils.isEmpty(activeRecommendationName)) markHandled(activeRecommendationName);
        if (!TextUtils.isEmpty(activeRecommendationFingerprint)) {
            handledRecommendationFingerprints.add(activeRecommendationFingerprint);
        }
        activeRecommendationName = null;
        activeRecommendationFingerprint = null;
        carriedRecommendationName = null;
    ...
    private void attemptAutomaticSend(String expectedName, long expectedEpoch, int attempt) {
        ...
        if (sendClickAcceptedForPending) {
            mainHandler.postDelayed(() -> confirmAutomaticSend(expectedName, expectedEpoch, 0), 120);
            return;
        }
        ...
        boolean clickedRealSendNode = false;
        ...
        if (freshRoot != null && isSoyoRoot(freshRoot)) {
            ...
            if (freshEditor != null) {
                ...
                clickedRealSendNode = clicked;
                // Koordinata kör tıklamayı ilk denemelerde yapma. Kullanıcının anlattığı
                // takılmada asıl sorun Flutter'ın gönder düğümünü kaydırma olmadan vermemesi;
                // önce otomatik uyandırma hareketlerinin çalışmasına fırsat ver.
                if (!clicked && attempt >= 6 && editorRect != null && !editorRect.isEmpty()) {
        ...
        if (clicked) {
            // Gerçek erişilebilirlik gönder düğümü tıklandıysa ikinci kez asla basma.
            // Sadece koordinat gesture'ı kabul edildiyse bunu "gönderildi" kanıtı sayma;
            // confirm akışı editörün temizlenmesini / mesajın görünmesini doğrulasın.
            sendClickAcceptedForPending = clickedRealSendNode;
            mainHandler.postDelayed(() -> confirmAutomaticSend(expectedName, expectedEpoch, 0), 150);
            return;
        }

        if (attempt + 1 < AUTO_SEND_MAX_ATTEMPTS) {
            // Editör görünür olduğu halde Flutter gönder düğmesini erişilebilirlik ağacına
            // koymadıysa kullanıcının yaptığı elle kaydırmanın karşılığını otomatik üret.
            // Aynı turda tıklamıyoruz; hareket sonrası yeni node ağacını tekrar buluyoruz.
            if (attempt == 2 || attempt == 5) {
                nudgeChatUi();
            }
            mainHandler.postDelayed(() -> attemptAutomaticSend(expectedName, expectedEpoch, attempt + 1),
                    attempt == 2 || attempt == 5 ? 320L : AUTO_SEND_RETRY_MS);
            return;
        }
    ...
    private void confirmAutomaticSend(String expectedName, long expectedEpoch, int attempt) {
        ...
        if (attempt < 8) {
            mainHandler.postDelayed(() -> confirmAutomaticSend(expectedName, expectedEpoch, attempt + 1), 180);
        } else if (sendClickAcceptedForPending) {
            // Android tıklamayı bir kez kabul ettiyse UI kanıtı gecikse bile ikinci kez GÖNDER'e
            // basma. Duplike mesaj riskinden kaçınmak için bu kişiyi tamamlanmış sayıp ilerle.
            finishAutomaticSend(expectedName, expectedEpoch, false);
        } else {
            autoSendScheduled = false;
            mainHandler.postDelayed(() -> scheduleAutomaticSend(expectedName, expectedEpoch), 260);
    ...
    private static class RecommendationTarget {
        ...
        RecommendationTarget(String name, AccessibilityNodeInfo chatButton, Rect chatRect, int score, boolean vip, String fingerprint) {
            this.name = name;
